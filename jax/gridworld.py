...
class DeterministicGridWorld: #deterministic env
    ...
    def idx_to_coord(self, idx: int) -> Position:
        return jnp.array((idx % self.cols, idx // self.cols))
    ...
    def next_coord(self, pos: Position, a: Action) -> Position:
        ...
        return jnp.clip(pos + self._actions[a], 0, [self.cols - 1, self.rows - 1])
...
def uniform_policy(env: DeterministicGridWorld) -> Policy:
   return jnp.ones((env.num_states, env.num_actions)) / env.num_actions

# Random deterministic policy
def make_random_policy(env: DeterministicGridWorld, seed=None) -> Policy:
    rng = jnp.random.RandomState(seed)
    policy = jnp.zeros((env.num_states, env.num_actions))
