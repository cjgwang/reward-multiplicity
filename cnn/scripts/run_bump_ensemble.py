...
"""
STARC Diversity Ensemble Experiment
====================================
Dataset: fixed star at (5,5). Both interpretations fit training data perfectly:
  - "Corner"      : reward = 1 when agent at (5,5), regardless of star
  - "Follow star" : reward = 1 when agent == star, wherever the star is

Model 0: trained with plain MSE  → naturally learns some reward function
Model 1: trained with MSE - alpha * STARC_dist(M1, M0)  → pushed to be diverse

Outputs: PNG plots saved to ./results/
"""
import sys, os
...
ROWS, COLS     = 7, 7
CHANNELS       = 3
STAR_POS       = (5, 5)        # fixed star position
N_POS, N_NEG   = 2000, 2000
HIDDEN_CHAN     = 4
FC_HIDDEN       = 32
EPOCHS_0        = 150           # model 0 (MSE only)
EPOCHS_1        = 150           # model 1 (MSE - STARC)
LR              = 1e-3
BATCH_SIZE      = 64
ALPHA           = 0.1           # weight on STARC term
GAMMA           = 0.9
SEED_0          = 0
SEED_1          = 1
...
print(f"Config: H={ROWS} W={COLS} star={STAR_POS} alpha={ALPHA} gamma={GAMMA}")
...
def compute_true_rewards(state_index_map, mode="corner"):
    """Compute ground-truth rewards for all canonical states."""
...
print("\n── Building datasets ──")
set_seed(42)
X, y = generate_balanced_dataset(star_pos=STAR_POS, n_pos=N_POS, n_neg=N_NEG, H=ROWS, W=COLS)
N = len(X)
n_val = int(0.1 * N)
...
print(f"Full domain S = {env.num_states} states")
...
# ──────────────────────────────────────────────
# Precompute STARC matrices (uniform policy)
# ──────────────────────────────────────────────
...
print(f"Model 0 full-domain r: min={r0_full.min():.3f} max={r0_full.max():.3f}")
...
def bump_function(x, center=0.0, width=1.3, eps=1e-12):
    ...
    return torch.where(mask, torch.exp(-1.0 / denom), torch.zeros_like(x))

print(f"\n── Training Model 1 (MSE - {ALPHA}*STARC, seed={SEED_1}) ──")
set_seed(SEED_1)
...
for ep in range(1, EPOCHS_1 + 1):
    model1.train()
    total_loss = total_mse = total_starc = 0.0
    n = 0
    for imgs, labels in train_loader:
        ...
        # Full-domain forward (gradient flows here for STARC)
...
# ──────────────────────────────────────────────
# Compute final STARC distance between the two models
# ──────────────────────────────────────────────
...
print(f"\n── Final STARC distance (M0 vs M1): {final_starc:.4f}  (max possible = 2.0) ──")

# Compute reference distances for "true" rewards
...
print(f"\nSTARC distances to true reward functions:")
print(f"  True corner vs True star :  {d_true:.4f}")
print(f"  Model 0 vs True corner   :  {d_m0_corner:.4f}")
print(f"  Model 0 vs True star     :  {d_m0_star:.4f}")
print(f"  Model 1 vs True corner   :  {d_m1_corner:.4f}")
print(f"  Model 1 vs True star     :  {d_m1_star:.4f}")
...
def r_to_heatmaps(r_state, state_index_map, rows, cols):
    """r_state (S,) -> heatmaps (K, rows, cols), star_positions list of (x,y)"""
    S = state_index_map.shape[0]
...
def plot_heatmap_grid(heatmaps, star_positions, rows, cols, title, save_path,
    ...
    ncols_fig = cols   # one column per x-position of star = 7
    nrows_fig = rows   # one row per y-position of star = 7
    fig, axes = plt.subplots(nrows_fig, ncols_fig, figsize=(ncols_fig * 1.6, nrows_fig * 1.5))

    if vmin is None: vmin = float(np.nanmin(heatmaps))
    if vmax is None: vmax = float(np.nanmax(heatmaps))
    norm = Normalize(vmin=vmin, vmax=vmax)

    # Map star position to panel
    ...
    for gy in range(rows):
        for gx in range(cols):
            ...
            if star in star_to_idx:
                ...
                # mark star position
                ax.scatter([gx], [gy], s=60, marker='*', c='yellow', edgecolors='white', linewidths=0.5, zorder=4)
...
# ──────────────────────────────────────────────
print("\n── Generating plots ──")
hm_corner, sp = r_to_heatmaps(r_corner, state_index_map, ROWS, COLS)
hm_star,   _  = r_to_heatmaps(r_star,   state_index_map, ROWS, COLS)
hm_m0,     _  = r_to_heatmaps(r0_full,  state_index_map, ROWS, COLS)
hm_m1,     _  = r_to_heatmaps(r1_full,  state_index_map, ROWS, COLS)

# Use a shared vmin/vmax across all model predictions
...
plot_heatmap_grid(hm_corner, sp, ROWS, COLS,
                  title="True 'Corner' reward  (r=1 iff agent at (5,5), any star)",
                  save_path="results/true_corner.png", vmin=0, vmax=1, cmap="Blues")

plot_heatmap_grid(hm_star, sp, ROWS, COLS,
                  title="True 'Follow Star' reward  (r=1 iff agent == star)",
                  save_path="results/true_star.png", vmin=0, vmax=1, cmap="Greens")

plot_heatmap_grid(hm_m0, sp, ROWS, COLS,
                  title=f"Model 0 (MSE only, seed={SEED_0})\n"
                        f"STARC to corner={d_m0_corner:.3f}  to star={d_m0_star:.3f}",
                  save_path="results/model0.png", vmin=vmin_all, vmax=vmax_all)

plot_heatmap_grid(hm_m1, sp, ROWS, COLS,
                  title=f"Model 1 (MSE − {ALPHA}·STARC, seed={SEED_1})\n"
                        f"STARC to corner={d_m1_corner:.3f}  to star={d_m1_star:.3f}",
                  save_path="results/model1.png", vmin=vmin_all, vmax=vmax_all)

# ── Summary figure: single star row for quick comparison ──
fig, axes = plt.subplots(4, COLS, figsize=(COLS * 1.8, 4 * 1.5))
labels_rows = [
    ("True Corner",       hm_corner, 0.0, 1.0, "Blues"),
    ("True Follow Star",  hm_star,   0.0, 1.0, "Greens"),
    (f"Model 0 (MSE)",    hm_m0,     vmin_all, vmax_all, "viridis"),
    (f"Model 1 (STARC)",  hm_m1,     vmin_all, vmax_all, "plasma"),
]

# Show only the row where star y=5 (the diagonal row containing (5,5))
star_y_row = STAR_POS[1]  # y=5
...
for row_i, (row_label, hm, vm, vx, cm) in enumerate(labels_rows):
    ...
    for gx in range(COLS):
        ax = axes[row_i, gx]
        star = (gx, star_y_row)
        if star in {sp[k]: k for k, sp_k in enumerate(sp)}.keys() if False else True:
            star_to_idx = {s: i for i, s in enumerate(sp)}
            if star in star_to_idx:
                k = star_to_idx[star]
                ax.imshow(hm[k], origin='upper', cmap=cm, norm=norm, aspect='equal')
                ax.scatter([gx], [star_y_row], s=60, marker='*', c='yellow', edgecolors='white', linewidths=0.5, zorder=4)
        ax.set_xticks([]); ax.set_yticks([])
        ax.set_title(f"★=({gx},{star_y_row})", fontsize=6, pad=1)
        if gx == 0:
            ax.set_ylabel(row_label, fontsize=7, rotation=0, labelpad=60, va='center')

fig.suptitle(f"Comparison — star row y={star_y_row}\n"
             f"STARC(M0,M1)={final_starc:.3f}  True STARC(corner,star)={d_true:.3f}",
             fontsize=10)
plt.tight_layout()
plt.savefig("results/comparison_row.png", dpi=150, bbox_inches='tight')
plt.close(fig)
print("Saved: results/comparison_row.png")
...
ax.set_title(f"STARC distances (alpha={ALPHA})")
ax.legend(fontsize=8)
plt.tight_layout()
plt.savefig("results/starc_distances.png", dpi=150, bbox_inches='tight')
plt.close(fig)
print("Saved: results/starc_distances.png")
