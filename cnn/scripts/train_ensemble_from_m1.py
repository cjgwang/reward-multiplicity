...
MSE + STARC loss against a pre-trained frozen ensemble_model_1.pth.

Usage:
    python scripts/train_ensemble_from_m1.py --full_domain grid_full_domain.npz
    python scripts/train_ensemble_from_m1.py --alpha 0.00001 --ensemble_size 3
"""
import sys, os
...
def main():
    ...
    parser.add_argument("--frozen_model", type=str, default="ensemble_model_1.pth",
                        help="Path to the pre-trained model to use as the frozen reference")
    ...
    # --- Load frozen reference model (ensemble_model_1.pth) ---
    print(f"\nLoading frozen reference model from {args.frozen_model}...")
    frozen_model = CNN(in_channels=args.channels, hidden_channels=args.hidden_channels,
                       H=args.rows, W=args.cols).to(device)
    checkpoint = torch.load(args.frozen_model, map_location=device)
    frozen_model.load_state_dict(checkpoint["model_state_dict"])
    frozen_model.eval()

    with torch.no_grad():
        r_frozen = frozen_model(images_full_t).reshape(S)
    C_frozen_flat = r_state_to_cnorm_flat(r_frozen, starc_pre, args.gamma).detach()
    print(f"Frozen model C shape: {C_frozen_flat.shape}")

    # --- Train ensemble members against frozen ---
    for m_idx in range(args.ensemble_size):
        ...
        torch.save({"model_state_dict": model.state_dict()}, f"ensemble_from_m1_model_{m_idx}.pth")

    print(f"\nDone. Saved {args.ensemble_size} model checkpoints: ensemble_from_m1_model_0.pth ... ensemble_from_m1_model_{args.ensemble_size-1}.pth")
